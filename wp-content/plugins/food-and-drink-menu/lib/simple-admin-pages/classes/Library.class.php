...
class sapLibrary_2_0_a_7 {
	...
	private function get_setting_classname( $type ) {
		...
		switch( $type ) {
			...
			case 'warningtip' :
				require_once('AdminPageSetting.WarningTip.class.php');
				return $this->get_versioned_classname( 'sapAdminPageSettingWarningTip' );
	...
	public function enqueue_scripts() {
		...
		foreach ( $this->pages as $page_id => $page ) {
			...
			if ( strpos( $screen->base, $page_id ) !== false ) {
				...
				wp_enqueue_script( 'fdm-admin-toggle-js', FDM_PLUGIN_URL . '/assets/js/admin-toggle.js', array( 'jquery' ), '2.0.0', true );
				wp_enqueue_style( 'fdm-admin-spectrum-css', $this->lib_url . 'css/spectrum.css', array(), '2.0.0' );
				wp_enqueue_script( 'fdm-admin-spectrum-js', $this->lib_url . 'js/spectrum.js', array( 'jquery' ), '2.0.0', true );
				wp_enqueue_script( 'fdm-admin-settings-js', $this->lib_url . 'js/admin-settings.js', array( 'jquery' ), '2.0.0', true );
