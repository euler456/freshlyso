...
class fdmOrderManager {
	...
	public function __construct() {
		...
		add_action( 'fdm_order_accepted_' . FDM_ORDER_POST_TYPE, array( $this, 'send_customer_order_notification' ), 10, 2 );
	}
	...
	public function submit_order( $args ) {
		...
		if ( $order->post_status == 'fdm_order_received' ) {
			$order_notification = new fdmOrderNotification ( $order, array( 'target' => 'admin' ) );
	...
	public function send_customer_order_notification( $ID, $post ) {

		$order = new fdmOrderItem();

		$order->load( $post ); 

		$order_notification = new fdmOrderNotification( $order, array( 'target' => 'user' ) );
		$order_notification->prepare_notification();
		$order_notification->send_notification();
	}
	...
	public function get_recent_order_id() {
		...
		return isset( $_COOKIE['fdm_recent_order'] ) ? $_COOKIE['fdm_recent_order'] : 0;
	}
	...
	public function is_open_for_ordering() {
		...
		if ( ! isset( $this->is_open_for_ordering ) ) { $this->is_open_for_ordering = $this->determine_open_for_ordering(); }
	...
	public function determine_open_for_ordering() {
		...
		if ( ! $ordering_open ) { return $ordering_open; }
		...
		foreach ( $exceptions as $exception ) {
			if ( !empty( $exception['date'] ) and date('Y-m-d ', strtotime( $exception['date'] ) ) == $today_date ) {
				...
				if ( empty( $exception['time'] ) ) { $ordering_open = false; }						
					...
					if ( time() + $gmt_seconds_offset  > strtotime( $today_date . $exception['time']['start']) and time() + $gmt_seconds_offset  < strtotime( $today_date . $exception['time']['end'])) { $ordering_open = false; }
				}
			}
		}
