...
class fdmCustomPostTypes {
	...
	public function add_meta_boxes() {
		...
		if ( $fdm_controller->settings->get_setting('fdm-enable-ordering-options') ) { 
	...
	public function menu_item_row_actions( $actions, $post ) {
		...
		$url = admin_url( 'admin.php?page=' . FDM_MENUITEM_POST_TYPE . '&post=' . $post->ID . '&action=duplicate' );
	...
	public function show_menu_item_custom_fields() {
		...
		$fields = json_decode( html_entity_decode( $fdm_controller->settings->get_setting('fdm-custom-fields') ) );
	...
	public function fdmp_show_item_special() {
		...
		// Retrieve values for this if it exists
		$special = get_post_meta( $post->ID, 'fdm_item_special', true );
		...
		?>
			...
			<div class="fdm-source-input fdm-input-controls">
				<div class="fdm-input-control fdm-radio-control">
					<input type="radio" name="fdm_item_special" id="fdm_item_special_none" value="" <?php if ( !$special ) : ?> checked="checked"<?php endif; ?>>
					<label for="fdm_item_special_none"><?php echo __( 'None', 'food-and-drink-menu-pro' ); ?></label>
				</div>
				<div class="fdm-input-control fdm-radio-control">
					<input type="radio" name="fdm_item_special" id="fdm_item_special_sale" value="sale" <?php if ( $special == 'sale' ) : ?> checked="checked"<?php endif; ?>>
					<label for="fdm_item_special_sale"><?php echo __( 'On Sale', 'food-and-drink-menu-pro' ); ?></label>
				</div>
				<div class="fdm-input-control fdm-radio-control">
					<input type="radio" name="fdm_item_special" id="fdm_item_special_offer" value="offer" <?php if ( $special == 'offer' ) : ?> checked="checked"<?php endif; ?>>
					<label for="fdm_item_special_offer"><?php echo __( 'Special Offer', 'food-and-drink-menu-pro' ); ?></label>
				</div>
				<div class="fdm-input-control fdm-radio-control">
					<input type="radio" name="fdm_item_special" id="fdm_item_special_featured" value="featured" <?php if ( $special == 'featured' ) : ?> checked="checked"<?php endif; ?>>
					<label for="fdm_item_special_featured"><?php echo __( 'Featured', 'food-and-drink-menu-pro' ); ?></label>
				</div>
			</div>
		...
		if ( isset( $_POST['fdm_menu_item_flag_meta'] ) ) {
			...
			foreach ( $cat_keys as $key ) {
				if ( isset ( $_POST['fdm_menu_item_flag_meta'][$key] ) ) {
					$term_meta[$key] = $_POST['fdm_menu_item_flag_meta'][$key];
				}
			}
	...
	public function maybe_duplicate_item() {
		...
		if ( isset($_GET['page']) and $_GET['page'] == FDM_MENUITEM_POST_TYPE and isset($_GET['action']) and $_GET['action'] == 'duplicate' ) {
	...
	function duplicate_post( $post_id ) {
		...
		wp_set_post_terms( $new_post_id, $section_terms, 'fdm-menu-section' );

		$taxonomy_terms = get_the_terms( $post_id, 'fdm-menu-item-flag' );
		wp_set_post_terms( $new_post_id, $taxonomy_terms, 'fdm-menu-item-flag' );
