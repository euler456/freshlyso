...
<?php
	...
	public function get_all_default_columns() {
		...
		$columns = array(
			...
			// 'details' => __( 'Details', 'food-and-drink-menu' )
		);
	...
	public function column_default( $order, $column_name ) {
		...
		switch ( $column_name ) {
			case 'date' :
				...
				//$value .= '<div class="status"><span class="spinner"></span> ' . __( 'Loading', 'food-and-drink-menu' ) . '</div>';

				if ( $order->post_status !== 'trash' ) { //@to-do: allow modal editing/trashing of order
//					$value .= '<div class="actions">';
//					$value .= '<a href="#" data-id="' . esc_attr( $order->ID ) . '" data-action="edit">' . __( 'Edit', 'food-and-drink-menu' ) . '</a>';
//					$value .= ' | <a href="#" class="trash" data-id="' . esc_attr( $order->ID ) . '" data-action="trash">' . __( 'Trash', 'food-and-drink-menu' ) . '</a>';
//					$value .= '</div>';
				}
			...
			case 'items' :
				...
				foreach ( $order_items as $order_item ) {
					...
					foreach ( $order_item->selected_options as $selected_option ) {
						if ( array_key_exists($selected_option, $ordering_options ) ) { $value .= '<li>' . $ordering_options[ $selected_option ]['name'] . '</li>'; }
					}
					if ( ! empty( $order_item->selected_options ) ) { $value .= '</ul>'; }
				...
				$value .= '
					<div class="fdm-admin-order-total-price">
						<span class="label">' . __( 'Total:', 'food-and-drink-menu' ) . ' </span><span class="value">' . fdm_format_price( $order_total_price ) . '</span>
					</div>
				';
	...
	public function add_notification() {
		...
		if ( !empty( $_GET['status'] ) ) {
			...
			if ( $status == 'trash' ) {
				...
				$notifications['status'] = sprintf( _x( "You're viewing orders that have been marked as %s.", 'Indicates which order status is currently being filtered in the list of orders.', 'food-and-drink-menu' ), $order_statuses[ $_GET['status'] ]['label'] );
			}
		}

		/*if ( !empty( $this->filter_start_date ) || !empty( $this->filter_end_date ) ) {
			$notifications['date'] = sprintf( _x( 'Only orders from %s are being shown.', 'Notification of order date range, eg - orders from 2014-12-02-2014-12-05', 'food-and-drink-menu' ), $this->get_current_date_range() );
